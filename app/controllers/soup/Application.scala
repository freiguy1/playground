...
object Application extends Controller with securesocial.core.SecureSocial {
  ...
  def uuidUpdateEntry(uuid: String) = Action { implicit request => 
    Accessor.getEntryByUuid(uuid).map { dbEntry =>  
      ...
      Ok(views.html.soup.update(uuid, entryForm.fill(dto)))
    }.getOrElse(NotFound)
  }
  ...
  def uuidUpdateEntrySubmit(uuid: String) = Action { implicit request =>
    entryForm.bindFromRequest.fold(
      formWithErrors => {
        BadRequest(views.html.soup.update(uuid, formWithErrors))
      },
      updatedEntry => {
        Accessor.getEntryByUuid(uuid).map( dbEntry => { 
          Accessor.updateEntry(updatedEntry.toEntry(dbEntry.entryId, dbEntry.uuid))
          Redirect(controllers.soup.routes.Application.index)
        }).getOrElse(NotFound)
      }
    )
  }
  ...
  def admin = SecuredAction(new SoupAdminAuth) { implicit request =>
    Ok(views.html.soup.admin());
  }

  def results = SecuredAction(new SoupAdminAuth) { implicit request =>
    ...
    Ok(views.html.soup.results(results.sortBy(r => 0 - r._3)));
  }

  def getAdminEntries() = SecuredAction(new SoupAdminAuth) { implicit request => 
    Ok(Json.toJson(Accessor.getEntries))
  }

  def clearVotes() = SecuredAction(new SoupAdminAuth) { implicit request => 
    Accessor.clearVotes
    NoContent
  }

  def updateEntry(entryId: Int) = SecuredAction(true, new SoupAdminAuth)(parse.json) { implicit request => 
  ...
  def deleteEntry(entryId: Int) = SecuredAction(true, new SoupAdminAuth) { implicit request =>
